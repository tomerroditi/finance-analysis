...
down_revision: str | Sequence[str] | None = "7beb5c98691f"
...
def upgrade() -> None:
    ...
    if "insurance_policy_id" not in columns:
        with op.batch_alter_table("investments") as batch_op:
            batch_op.add_column(
                sa.Column("insurance_policy_id", sa.String(), nullable=True)
            )
            batch_op.create_unique_constraint(
                "uq_investments_insurance_policy_id", ["insurance_policy_id"]
            )
