...
installed; these endpoints only read the history table, so they stay
available on the hosted demo. That is the difference between a Data Sources
page that shows when each source last synced and one where every card claims
it has never synced.
"""
