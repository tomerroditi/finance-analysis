...
from backend.repositories.credentials_repository import CredentialsRepository, CREDENTIALS_PATH
...
class CredentialsService:
    ...
    @staticmethod
    def generate_keyring_key(service: str, provider: str, account: str, field: str) -> str:
                ...
                for account, fields in accounts.items():
                    for field, value in fields.items():
                        if field == LoginFields.PASSWORD.value:
                            ...
                            credentials[service][provider][account][field] = password or ""
    ...
    def delete_account(self, service: str, provider: str, account: str) -> None:
        ...
        if (service in credentials and 
            provider in credentials[service] and 
            account in credentials[service][provider]):
            del credentials[service][provider][account]
