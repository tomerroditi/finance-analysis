...
class TransactionsService:
    ...
    def add_transaction(self, transaction: dict, service: Literal['cash', 'manual_investments']) -> bool:
        ...
        if service == 'cash':
            ...
            raise ValueError("Only 'cash' and 'manual_investments' services are supported.")
    ...
    def delete_transaction_by_id(self, transaction_id: str) -> bool:
        ...
        return self.transactions_repository.cash_repo.delete_transaction_by_id(transaction_id)

    def get_transactions_by_tag(self, category: str, tag: Optional[str] = None) -> pd.DataFrame:
    ...
    def get_table_for_analysis(
        ...
        return result_df[analysis_cols].reset_index(drop=True) if all(c in result_df.columns for c in analysis_cols) else result_df
