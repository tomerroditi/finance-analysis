...
def _choice(enum: type[Enum], raw: Any, key: str) -> Any:
    ...
    try:
        ...
        raise ValidationException(
            f"{key} must be one of {options}, got {raw!r}"
        ) from exc
...
def _flow(payload: dict, prefix: str, index: int, default_end: str) -> CashFlow:
    return CashFlow(
        ...
        start_type=_choice(
            StartType,
            payload.get(f"{prefix}StartType{index}", "now"),
            f"{prefix}StartType{index}",
        ),
        start_date=_date(payload, f"{prefix}StartDate{index}"),
        end_type=_choice(
            EndType,
            payload.get(f"{prefix}EndType{index}", default_end),
            f"{prefix}EndType{index}",
        ),
...
def _portfolio(payload: dict, index: int) -> Portfolio:
    return Portfolio(
        ...
        designation=_choice(
            PortfolioDesignation,
            payload.get(f"portfolioDesignation{index}", "withdraw"),
            f"portfolioDesignation{index}",
        ),
        kind=_choice(
            PortfolioType,
            payload.get(f"portfolio_type{index}", "portfolio"),
        ...
        lot_method=_choice(
            LotMethod,
            payload.get(f"portfolio_fifo_lifo{index}", "flat"),
...
def _pension(
    payload: dict, suffix: str, tactic_key: str, end_key: str
) -> Pension | None:
    """Build the person's pension fund, empty or not.
    ...
    return Pension(
        ...
        work_start_year=int(_num(payload, f"work_start_year{suffix}", None) or 0)
        or None,
    )
...
def plan_from_reference(payload: dict) -> Plan:
    ...
    return Plan(
        person=person,
        partner=partner,
        base_problem=_choice(
            BaseProblem, payload.get("base_problem", "retire_asap"), "base_problem"
        ),
        ...
        draw_keren_before_portfolio=payload.get("prati_hishtalmut_order")
        == "hishtalmut",
        ...
        kranot_hishtalmut=[
            KerenHishtalmut(
                ...
                kind=_choice(
                    KerenType, payload.get(f"kerenType{i}", "maslulit"), f"kerenType{i}"
                ),
                annual_fee_pct=_req(payload, f"kerenFee{i}", 0.6),
                end_type=_choice(
                    EndType, payload.get(f"kerenEndType{i}", "fire"), f"kerenEndType{i}"
                ),
        ...
        loans=[
            Loan(
                ...
                kind=_choice(
                    LoanType, payload.get(f"debtType{i}", "spitzer"), f"debtType{i}"
                ),
            )
