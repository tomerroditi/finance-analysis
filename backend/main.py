...
# Only /api/* is guarded: the static SPA shell contains no data, and the
# frontend picks the token up from a one-time `?apiToken=` URL parameter.
@app.middleware("http")
async def require_token_for_remote_clients(request: Request, call_next):
    """Require a bearer token on /api requests from non-local clients."""
    if (
