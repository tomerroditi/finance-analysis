...
_cors_origins = [
    ...
    for origin in os.getenv(
        "CORS_ORIGINS",
        "" if _environment == "production" else "http://localhost:5173,http://127.0.0.1:5173",
    ).split(",")
    if origin.strip()
]
...
# response measured 1233 KB -> 111 KB). Registered first, so it runs
# innermost — the ``@app.middleware`` guards below wrap it; 500 bytes is the
# usual floor below which framing costs more than it saves.
...
async def enforce_same_origin_for_writes(request: Request, call_next):
    ...
    if not auth.origin_allowed(
        origin, request.headers.get("host"), _cors_origins
    ):
        logger.warning(
