"""
Investments repository with pure SQLAlchemy (no Streamlit dependencies).

This module provides data access for investment tracking operations.
"""
...
class InvestmentsRepository:
    ...
    def _assure_table_exists(self) -> None:
        ...
        # Migration: Add interest_rate_type column if missing
        result = self.db.execute(text(f"PRAGMA table_info({self.table})"))
        columns = [row[1] for row in result.fetchall()]
        
        if self.interest_rate_type_col not in columns:
            self.db.execute(text(
                f"ALTER TABLE {self.table} ADD COLUMN {self.interest_rate_type_col} TEXT DEFAULT 'fixed'"
            ))
            self.db.commit()
    ...
    def update_investment(self, investment_id: int, **fields) -> None:
        ...
        result = self.db.execute(
            text(f"UPDATE {self.table} SET {set_clause} WHERE {self.id_col} = :id"),
            fields
        )
        self.db.commit()
    ...
    def close_investment(self, investment_id: int, closed_date: str) -> None:
        ...
        self.db.execute(
            ...
            """),
            {'id': investment_id, 'closed_date': closed_date}
        )
        self.db.commit()
