...
| Layer | File | Job |
|---|---|---|
| Service worker | generated by `vite-plugin-pwa` (`generateSW`) — config in `frontend/vite.config.ts` | Precaches the static shell, runtime-caches `/api` GETs (NetworkFirst, 4 s timeout), excludes credentials/scraping/backups |
