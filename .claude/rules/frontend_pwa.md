...
| QueryClient + persister | `frontend/src/queryClient.ts` | Owns `QueryClient`, persists the React Query cache to IndexedDB via `idb-keyval`, installs the app-wide mutation cache from `queryInvalidation.ts` (cancels pre-write reads, then sweeps) |
...
| Update check | `frontend/src/hooks/useServiceWorkerUpdateCheck.ts` | Polls `registration.update()` so an open tab notices a deploy — nothing else does |
...
## Nothing looks for a new build unless we do

`registerType: "prompt"` decides what happens **once a new service worker is
found**. It does not go looking. vite-plugin-pwa has no poll of its own, and
the browser re-fetches `sw.js` on only two occasions: a **navigation** request
inside the worker's scope, or a functional event once 24 h have passed.

An SPA performs neither. Route changes are client-side history pushes, and
every `/api` call is a `fetch` the worker *handles* — none of it is a
navigation. So a tab left open on the dashboard could sit on a build from days
ago and never raise the reload toast. This was not a slow update path; there
was no update path.

`hooks/useServiceWorkerUpdateCheck.ts` supplies the missing poll: one
`registration.update()` a minute, plus an immediate check when a backgrounded
tab returns or connectivity comes back. Checks are skipped while the tab is
hidden or offline, and while an update is already installing or waiting (the
toast is up — asking again changes nothing). A registration's default
`updateViaCache: "imports"` means the browser bypasses the HTTP cache for the
worker script, so the check sees a new build as soon as it is on disk.

The other half of the delay is on the server: see `plan_redeploy` in
`.claude/scripts/prod_server.py`, which keeps a release's `bump:` commit from
costing a full `npm ci` and bundle rebuild.

None of this is reachable from e2e — `devOptions.enabled: false` means the
service worker only exists in a production build, so the dev server Playwright
drives never registers one. The guard is
`hooks/useServiceWorkerUpdateCheck.test.ts` (vitest).

## When you add a new mutation

The app-wide mutation cache (`queryInvalidation.ts`, installed by
`queryClient.ts`) already triggers a debounced
`queryClient.invalidateQueries()` 200 ms after every mutation settles, and
cancels any read that was in flight when the write began. So:
...
- Do **not** remove the periodic `registration.update()` on the assumption
  that `registerType: "prompt"` or the browser's own 24 h check covers it.
  Neither looks for a new build in an SPA — see "Nothing looks for a new
  build unless we do".
...
│   │   └── useServiceWorkerUpdateCheck.ts         # Polls registration.update() (an SPA never navigates)
│   ├── services/
