# Scraper Date Fixup Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Fix off-by-one date bug in onezero/isracard transactions from the old Node.js scraper and remove resulting duplicates.

**Architecture:** Standalone Python script using raw SQL via SQLAlchemy. Backs up DB, fixes dates, deduplicates, re-links splits. Dry-run by default, `--apply` to commit.

**Tech Stack:** Python, SQLAlchemy (raw SQL), SQLite, argparse

---

### Task 1: Create the fixup script with DB backup and argument parsing

**Files:**
- Create: `scripts/fix_scraper_dates.py`
...
def apply_fixes(
    ...
    # Build set of old unique_ids that are duplicates (need date fix + new row deleted)
    ...
    # 1. Fix dates on ALL old transactions (both duplicates and non-duplicates)
    for old_txn in old_txns:
        corrected_date = shift_date_forward(old_txn["date"])
        conn.execute(
    ...
    # 2. Re-link splits from new (about-to-be-deleted) rows to old (surviving) rows
...
**Step 2: Dry-run the script against the real database**

Run: `python scripts/fix_scraper_dates.py`

Verify:
- Counts look reasonable
- Sample duplicates show the expected +1 day pattern
- No errors

**Step 3: Apply the fix**

Run: `python scripts/fix_scraper_dates.py --apply`

Verify:
- Backup file created at `~/.finance-analysis/data.db.bak-*`
- Counts match the dry run
- "All changes committed successfully" printed
...
Verify:
- 0 duplicates found (all resolved)
- Old transaction counts are 0 (all dates already fixed, none before cutoff)
- No new transactions orphaned

**Step 5: Commit**

```bash
git add scripts/fix_scraper_dates.py docs/plans/2026-03-08-scraper-date-fixup-design.md docs/plans/2026-03-08-scraper-date-fixup-plan.md
git commit -m "fix: add one-time script to fix onezero/isracard date bug and remove duplicates"
```
