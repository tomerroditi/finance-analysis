...
export function TransactionEditorModal({ transaction, onClose, onSuccess }: TransactionEditorModalProps) {
    const isManual = transaction.source === 'cash' || transaction.source === 'manual_investment';
    ...
    const availableTags = formData.category && categories ? categories[formData.category] || [] : [];
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    {!isManual && (
                        <div className="bg-blue-500/10 border border-blue-500/20 text-blue-400 p-3 rounded-xl text-xs mb-4">
                            Note: For bank/credit card transactions, only Category and Tag can be modified to maintain data integrity.
                        </div>
                    )}
                        ...
                        <div className="grid grid-cols-2 gap-4 pt-2 border-t border-[var(--surface-light)] mt-4">
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) => setFormData({ ...formData, category: e.target.value, tag: '' })}
