...
export function RuleManager({ onClose }: RuleManagerProps) {
    ...
    const createMutation = useMutation({
        mutationFn: (rule: any) => {
            const payload = {
                ...
                conditions: [{ field: 'description', operator: 'contains', value: rule.description_contains }],
        ...
        onSuccess: (res) => {
            ...
            setSuccessMessage(`Rule created! ${res.data.tagged_count} transactions tagged.`);
    ...
    const updateMutation = useMutation({
        ...
        onSuccess: (res) => {
            ...
            setNewRule({ name: '', description_contains: '', category: '', tag: '' });
            setSuccessMessage(`Rule updated! ${res.data.tagged_count} transactions tagged.`);
            setTimeout(() => setSuccessMessage(null), 5000);
        }
    });
    ...
    const applyMutation = useMutation({
        ...
        onSuccess: (res) => {
            ...
            setSuccessMessage(`Success! ${res.data.tagged_count} transactions tagged.`);
            setTimeout(() => setSuccessMessage(null), 5000);
        }
    });

    const availableTags = newRule.category && categories ? categories[newRule.category] || [] : [];
            ...
            <div className="bg-[var(--surface)] border border-[var(--surface-light)] rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
                <div className="px-6 py-4 border-b border-[var(--surface-light)] flex items-center justify-between bg-[var(--surface-light)]/20">
                    <div>
                        ...
                        <p className="text-sm text-[var(--text-muted)]">Automatically tag transactions based on their description</p>
                    </div>
                ...
                <div className="p-6 overflow-y-auto flex-1">
                    <div className="flex gap-4 mb-4">
                        ...
                        >
                            <Play size={18} /> {applyMutation.isPending ? 'Applying...' : 'Apply Rules Now'}
                        </button>
                    </div>
                        ...
                        <div className="mb-6 p-4 rounded-xl bg-[var(--surface-base)] border border-[var(--primary)]/30 animate-in slide-in-from-top-2 duration-200">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <div className="space-y-1.5 text-left">
                                    <label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest ml-1">Rule Name</label>
                                    <input
                                        type="text"
                                ...
                                <div className="space-y-1.5 text-left">
                                    <label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest ml-1">Contains</label>
                                    <input
                                        type="text"
                                        placeholder="e.g. McDonald's"
                                        className="w-full bg-[var(--surface)] border border-[var(--surface-light)] rounded-lg px-3 py-2 text-sm outline-none focus:border-[var(--primary)]"
                                        value={newRule.description_contains}
                                        onChange={(e) => setNewRule({ ...newRule, description_contains: e.target.value })}
                                    />
                                </div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-1.5 text-left">
                                    <label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest ml-1">Category</label>
                                    <select
                                        className="w-full bg-[var(--surface)] border border-[var(--surface-light)] rounded-lg px-3 py-2 text-sm outline-none focus:border-[var(--primary)]"
                                        value={newRule.category}
                                        onChange={(e) => setNewRule({ ...newRule, category: e.target.value, tag: '' })}
                                    >
                                ...
                                <div className="space-y-1.5 text-left">
                                    <label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest ml-1">Tag</label>
                                    <select
                                ...
                                <button
                                    onClick={() => {
                                        if (editingRuleId !== null) {
                                            updateMutation.mutate({ id: editingRuleId, rule: newRule });
                                        } else {
                    ...
                    <div className="space-y-2">
                        {isLoading ? (
                            <div className="text-center py-8 text-[var(--text-muted)]">Loading rules...</div>
                        ) : rules?.length === 0 ? (
                            ...
                            rules.map((rule: any) => (
                                <div key={rule.id} className="group flex items-center justify-between p-4 rounded-xl bg-[var(--surface-base)]/50 border border-[var(--surface-light)] hover:border-[var(--primary)]/30 transition-all">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2">
                                                <span className="text-sm font-bold text-white">If includes:</span>
                                            ...
                                            <div className="flex items-center gap-2 mt-1">
                                                <span className="text-xs text-[var(--text-muted)]">Set Category to</span>
                                                <span className="text-xs font-semibold px-2 py-0.5 rounded bg-blue-500/10 text-blue-400">{rule.category}</span>
                                                {rule.tag && (
                                                    <>
                                                        <span className="text-xs text-[var(--text-muted)]">& Tag to</span>
                                                        <span className="text-xs font-semibold px-2 py-0.5 rounded bg-purple-500/10 text-purple-400">{rule.tag}</span>
                                                    </>
                                                )}
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        <button
                                            onClick={() => {
                                                const descriptionCondition = rule.conditions?.find((c: any) => c.field === 'description');
                                                setNewRule({
                                                    name: rule.name,
                                                    description_contains: descriptionCondition?.value || '',
                                                    category: rule.category,
                                                    tag: rule.tag
                                                });
