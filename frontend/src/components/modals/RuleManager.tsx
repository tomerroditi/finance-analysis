...
import { X, Plus, Trash2, ShieldCheck, Play } from 'lucide-react';
...
export function RuleManager({ onClose }: RuleManagerProps) {
    ...
    const [newRule, setNewRule] = useState({ description_contains: '', category: '', tag: '' });
    ...
    const createMutation = useMutation({
        mutationFn: (rule: any) => taggingApi.createRule(rule),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['tagging-rules'] });
            setIsCreating(false);
            setNewRule({ description_contains: '', category: '', tag: '' });
        }
    });
    ...
    const applyMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            alert('Rules applied successfully to all untagged transactions!');
        }
    });
                ...
                <div className="p-6 overflow-y-auto flex-1">
                    ...
                    {isCreating && (
                        <div className="mb-6 p-4 rounded-xl bg-[var(--surface-base)] border border-[var(--primary)]/30 animate-in slide-in-from-top-2 duration-200">
                            <h3 className="text-sm font-bold uppercase tracking-wider text-[var(--primary)] mb-4">Create New Rule</h3>
                            ...
                            <div className="flex gap-2 mt-4 justify-end">
                                <button onClick={() => setIsCreating(false)} className="px-4 py-2 text-sm font-medium hover:text-white transition-colors">Cancel</button>
                                <button
