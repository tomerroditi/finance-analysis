...
import { ArrowLeftRight, CheckCircle2, CircleDashed, Link, Calendar, CreditCard } from "lucide-react";
...
const RefundsView: React.FC = () => {
    ...
    if (isLoading) {
        return <div className="p-8 text-center text-[var(--text-muted)]">Loading refunds...</div>;
    }

    const renderRefundCard = (item: PendingRefund) => (
        <div key={item.id} className="bg-[var(--surface)] border border-[var(--surface-light)] rounded-xl overflow-hidden mb-4">
            ...
            <div className="p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[var(--surface-light)]/30">
                <div className="flex items-start gap-4">
                    <div className={`p-2 rounded-lg ${item.status === 'resolved' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'}`}>
                        {item.status === 'resolved' ? <CheckCircle2 size={20} /> : <CircleDashed size={20} />}
                    </div>
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <span className="font-semibold text-lg">{item.description || "Unknown Expense"}</span>
                ...
                <div className="flex flex-col items-end gap-1">
                    <div className="text-sm text-[var(--text-muted)]">Expected Refund</div>
                    <div className="text-xl font-bold font-mono text-[var(--text-primary)]">
                        {formatCurrency(item.expected_amount)}
                    </div>
                    {item.remaining !== undefined && item.remaining > 0 && item.remaining < item.expected_amount && (
            ...
            <div className="p-4 border-t border-[var(--surface-light)]">
                ...
                {!item.links || item.links.length === 0 ? (
                    ...
                    <div className="space-y-2 pl-2 border-l-2 border-[var(--surface-light)] ml-1">
                        {item.links.map(link => (
                            <div key={link.id} className="pl-4 py-1 flex justify-between items-center group">
                                <div className="flex items-center gap-2">
                                    <span className="text-emerald-400 font-mono font-medium">
                                        +{formatCurrency(link.amount)}
                                    </span>
                                    <span className="text-[var(--text-muted)]">•</span>
                                    <span className="text-sm">{link.description || "Refund Transaction"}</span>
        ...
        <div className="max-w-4xl mx-auto py-6 px-4">
            ...
            {(!refunds || refunds.length === 0) && (
                <div className="text-center py-12 text-[var(--text-muted)]">
                    ...
                    <p className="text-sm mt-2">Mark expenses as "Pending Refund" in your transactions list to track them here.</p>
                </div>
            )}
        </div>
    );
