...
import { RefreshCw, Smartphone, AlertCircle, PlayCircle, CheckSquare, Square, CheckCircle2, XCircle } from 'lucide-react';
...
export function ScrapingWidget() {
    ...
    const [runningScrapers, setRunningScrapers] = useState<Record<number, ScraperState>>({});

    // Fetch accounts
    const { data: accounts } = useQuery({
        queryKey: ['credentials-accounts'],
        queryFn: () => credentialsApi.getAccounts().then(res => res.data as Account[])
    });

    // Start Mutation
    const startMutation = useMutation({
        mutationFn: (acc: Account) => scrapingApi.start({
            service: acc.service,
    ...
    const tfaMutation = useMutation({
        mutationFn: ({ service, provider, account, code }: { service: string, provider: string, account: string, code: string }) =>
        ...
        const checkStatus = async () => {
            ...
            for (const scraper of activeScrapers) {
                try {
                    ...
                    if (newStatus !== scraper.status || (Date.now() - scraper.last_updated > 5000)) {
                        setRunningScrapers(prev => ({
                            ...prev,
                            [scraper.process_id]: {
                                ...scraper,
    ...
    const activeTfaScraper = Object.values(runningScrapers).find(s => s.status === 'waiting_for_2fa');
    const isRunning = Object.values(runningScrapers).some(s => s.status === 'in_progress');

    const getAccountId = (acc: Account) => `${acc.service}_${acc.provider}_${acc.account_name}`;
    ...
    const handleStartSelected = () => {
        ...
        const selectedList = accounts?.filter(acc => selectedAccounts.has(getAccountId(acc)));
    ...
    const getStatusIcon = (scraper: ScraperState | undefined) => {
        ...
        if (scraper.status === 'in_progress') return <RefreshCw size={16} className="animate-spin text-blue-400" />;
        if (scraper.status === 'waiting_for_2fa') return <Smartphone size={16} className="text-amber-400 animate-pulse" />;
        if (scraper.status === 'success') return <CheckCircle2 size={16} className="text-emerald-400" />;
        if (scraper.status === 'failed') return <XCircle size={16} className="text-red-400" />;
            ...
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-[var(--text-muted)]">Sync financial data</p>
                    </div>
                </div>
                {isRunning && (
                ...
                {Object.keys(runningScrapers).length > 0 && (
                    <div className="space-y-2 pt-4 border-t border-[var(--surface-light)] mt-auto">
                        <div className="flex items-center justify-between">
                            <h4 className="text-xs font-bold uppercase tracking-widest text-[var(--text-muted)]">Recent Activity</h4>
                        </div>
                        <div className="max-h-[120px] overflow-y-auto space-y-1 pr-1 custom-scrollbar">
                            {Object.values(
                                Object.values(runningScrapers)
                                    .reduce((acc, scraper) => {
                                ...
                                .map(scraper => (
                                    <div key={scraper.process_id} className="flex items-center justify-between p-2 rounded bg-black/20 text-[10px]">
                                        <span className="font-bold uppercase tracking-tight truncate mr-2">
                                            {scraper.account.provider} <span className="text-[var(--text-muted)]">/ {scraper.account.account_name}</span>
                                        </span>
