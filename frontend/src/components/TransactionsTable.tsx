...
export const TransactionsTable: React.FC<TransactionsTableProps> = ({
    ...
    const isAllSelected = useMemo(() => {
        ...
        return paginatedTransactions.every(tx => selectedIds.has(getTransactionId(tx)));
    ...
    const handleBulkTag = () => {
        ...
        const selectedTxs = transactions.filter(tx => selectedIds.has(getTransactionId(tx)));
    ...
    const handleBulkDelete = async () => {
        ...
        const selectedTxs = transactions.filter(tx => selectedIds.has(getTransactionId(tx)));
        const manualTxs = selectedTxs.filter(tx => tx.source?.includes('cash') || tx.source?.includes('manual_investment'));
    ...
    const SortableHeader = ({
        ...
        <th
            ...
            style={{ width }}
            className={`px-4 ${compact ? 'py-2' : 'py-3'} text-sm font-medium text-[var(--text-muted)] cursor-pointer group hover:text-white transition-colors ${align === 'right' ? 'text-right' : align === 'center' ? 'text-center' : 'text-left'
                }`}
        >
    ...
    if (sortedTransactions.length === 0 && !filterText) {
        return (
            <div className={`text-center text-[var(--text-muted)] ${compact ? 'py-4' : 'py-8'}`}>
    ...
    return (
        <>
            ...
            <div className="overflow-x-auto rounded-lg border border-[var(--surface-light)]">
                <table className="w-full text-sm text-left">
                    <thead className="bg-[var(--surface-light)] text-[var(--text-muted)] font-medium">
                        <tr>
                            {showSelection && (
                                <th className={`px-4 ${compact ? 'py-2' : 'py-3'} text-center`} style={{ width: '50px' }}>
                                    <input
                                        type="checkbox"
                            ...
                            <SortableHeader label="Category" sortKey="category" width="180px" />
                            <SortableHeader label="Amount" sortKey="amount" align="right" width="120px" />
                            {showActions && (
                                <th className={`px-4 ${compact ? 'py-2' : 'py-3'} text-center text-sm font-medium text-[var(--text-muted)]`} style={{ width: '100px' }}>
                                    Actions
                                </th>
                            )}
                        </tr>
                    ...
                    <tbody className="divide-y divide-[var(--surface-light)] bg-[var(--surface-base)]">
                        {paginatedTransactions.map((tx, idx) => {
                            ...
                            return (
                                ...
                                >
                                    {showSelection && (
                                        <td className={`px-4 ${compact ? 'py-2' : 'py-3'} text-center`}>
                                            <input
                                                type="checkbox"
                                                checked={isSelected}
                                    ...
                                    <td className={`px-4 ${compact ? 'py-2' : 'py-3'} whitespace-nowrap text-[var(--text-muted)]`}>
                                        {new Date(tx.date).toLocaleDateString()}
                                    </td>
                                    <td className={`px-4 ${compact ? 'py-2' : 'py-3'} truncate max-w-[150px]`} title={`${tx.provider || 'Manual'} - ${tx.account_name}`}>
                                    ...
                                    <td className={`px-4 ${compact ? 'py-2' : 'py-3'} text-[var(--text-default)] font-medium truncate max-w-[200px]`} title={getDescription(tx)}>
                                        {getDescription(tx)}
                                    </td>
                                    <td className={`px-4 ${compact ? 'py-2' : 'py-3'} text-[var(--text-muted)]`}>
                                        <span className="px-2 py-0.5 rounded-full bg-[var(--surface-light)] text-xs text-[var(--text-muted)] border border-[var(--surface-light)]">
                                            {tx.category || '-'} / {tx.tag || '-'}
                                        </span>
                                    </td>
                                    <td className={`px-4 ${compact ? 'py-2' : 'py-3'} text-right font-bold whitespace-nowrap ${tx.amount > 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                        ...
                        {sortedTransactions.length === 0 && (
                            <tr>
                                <td colSpan={columnCount} className={`px-4 ${compact ? 'py-4' : 'py-8'} text-center text-[var(--text-muted)]`}>
            ...
            {/* Pagination */}
            {totalPages > 0 && (
                <div className={`flex items-center justify-between ${compact ? 'mt-3 px-2' : 'mt-4 px-4 py-3 bg-[var(--surface-light)]/30 border-t border-[var(--surface-light)]'}`}>
                        ...
                        {isBulkTagging ? (
                            <div className="flex items-center gap-2">
                                <select
                                    ...
                                    onChange={(e) => setBulkTagData({ ...bulkTagData, category: e.target.value, tag: '' })}
                                >
                                ...
                                <select
                                    className="bg-[var(--surface-base)] border border-[var(--surface-light)] rounded-lg px-3 py-1.5 text-sm outline-none disabled:opacity-50"
                                    value={bulkTagData.tag}
                                    onChange={(e) => setBulkTagData({ ...bulkTagData, tag: e.target.value })}
                                    disabled={!bulkTagData.category}
                                >
                                    <option value="">Tag</option>
                                    {bulkTagData.category && categories?.[bulkTagData.category]?.map((tag: string) => (
                                ...
                                <button
                                    ...
                                    onClick={() => { setIsBulkTagging(false); setBulkTagData({ category: '', tag: '' }); }}
                                >
                                    <X size={20} />
                                </button>
                            </div>
