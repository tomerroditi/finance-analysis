...
export function DateRangePicker({ value, onChange }: DateRangePickerProps) {
    ...
    const presets = [
        ...
        {
            label: 'Current Year',
            getValue: () => ({ start: startOfYear(new Date()), end: endOfMonth(new Date()) }), // End of update is usually today or end of year
        },
        {
            label: 'Last Year',
            getValue: () => {
            ...
            >
                ...
                <span className="text-sm font-medium">
                    {value.start && value.end
                        ...
                        : (value.start === null && value.end === null ? 'All Time' : 'Select Date Range')}
                </span>
                ...
                <>
                    ...
                    <div className="absolute right-0 top-full mt-2 w-64 bg-[var(--surface)] border border-[var(--surface-light)] rounded-xl shadow-xl z-20 p-2">
                        ...
                        <div className="border-t border-[var(--surface-light)] my-2 pt-2">
                            <p className="text-xs text-[var(--text-muted)] px-3 mb-2">Custom Range</p>
                            <div className="grid grid-cols-2 gap-2 px-2">
                                <input
                                    ...
                                    onChange={(e) => onChange({ ...value, start: e.target.valueAsDate })}
                                />
                                <input
                                    type="date"
                                    className="w-full bg-[var(--background)] border border-[var(--surface-light)] rounded px-2 py-1 text-xs"
                                    value={value.end ? format(value.end, 'yyyy-MM-dd') : ''}
                                    onChange={(e) => onChange({ ...value, end: e.target.valueAsDate })}
                                />
                            </div>
                        </div>
                    </div>
