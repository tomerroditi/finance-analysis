  ...
  return (
    <header className="flex items-center justify-end px-8 py-4 mb-4 select-none">
      ...
      >
        <Play
          size={18}
          className={`transition-colors ${isDemoMode ? "text-emerald-500" : "text-[var(--text-muted)]"}`}
        />
        <span
          className={`text-sm font-bold ${isDemoMode ? "text-emerald-500" : "text-[var(--text-muted)]"}`}
        >
          Demo Mode
        </span>
        <div
          className={`w-8 h-4 rounded-full relative transition-colors ${isDemoMode ? "bg-emerald-500" : "bg-[var(--surface-light)]"}`}
        >
          <div
