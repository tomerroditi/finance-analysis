...
import { Search as SearchIcon, X, Receipt, Tags, ArrowRight } from 'lucide-react';
            ...
            Object.entries(categories).forEach(([cat, tags]: [string, any]) => {
                ...
                tags.forEach((tag: string) => {
                    if (tag.toLowerCase().includes(q)) {
                        results.push({ type: 'tag', title: tag, subtitle: `in ${cat}`, id: `tag-${cat}-${tag}` });
                    }
                });
            });
        }
        ...
        <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh] px-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
            ...
            >
                <div className="relative border-b border-[var(--surface-light)] bg-[var(--surface-light)]/20">
                    <SearchIcon className="absolute left-5 top-1/2 -translate-y-1/2 text-[var(--text-muted)]" size={20} />
                    <input
                        ref={inputRef}
                        type="text"
                    ...
                    <div className="absolute right-5 top-1/2 -translate-y-1/2 flex items-center gap-2">
                        <span className="px-1.5 py-0.5 rounded border border-[var(--surface-light)] bg-[var(--surface)] text-[10px] text-[var(--text-muted)] font-mono">ESC</span>
                ...
                <div className="max-h-[60vh] overflow-y-auto p-2">
                    ...
                    {searchResults.map((result, index) => (
                        <div
                            key={result.id}
                            className={`flex items-center justify-between p-4 rounded-xl cursor-pointer transition-all ${index === selectedIndex ? 'bg-[var(--primary)]/10 border border-[var(--primary)]/30' : 'hover:bg-[var(--surface-light)]/50 border border-transparent'
                                }`}
                            onMouseEnter={() => setSelectedIndex(index)}
                            onClick={() => handleSelect(result)}
                        >
                            <div className="flex items-center gap-4">
                                <div className={`p-2.5 rounded-xl ${result.type === 'transaction' ? 'bg-amber-500/10 text-amber-400' :
                                        result.type === 'category' ? 'bg-blue-500/10 text-blue-400' : 'bg-purple-500/10 text-purple-400'
                                    }`}>
                                    {result.type === 'transaction' ? <Receipt size={20} /> : <Tags size={20} />}
                                </div>
                                <div>
                                    <div className="text-sm font-bold text-white">{result.title}</div>
                                    {result.subtitle && (
                                        <div className="text-xs text-[var(--text-muted)] mt-0.5 lowercase tracking-tight">{result.subtitle}</div>
                                    )}
                                </div>
                            </div>
                            {index === selectedIndex && (
                ...
                {query.length === 0 && (
                    <div className="p-8 text-center">
                        ...
                        <p className="text-sm text-[var(--text-muted)] mt-1 max-w-xs mx-auto">
                            Search for anything across your financial dashboard. Categories, tags, or specific transactions.
                        </p>
                    </div>
                )}
            </div>
