...
export const ProjectBudgetView: React.FC = () => {
    ...
    const { data: projectDetails } = useQuery({
        ...
        queryFn: () => budgetApi.getProjectDetails(selectedProject).then((res) => res.data),
    ...
    const updateRuleMutation = useMutation({
        mutationFn: ({ id, rule }: { id: number; rule: any }) => budgetApi.updateRule(id, rule),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['projectDetails', selectedProject] });
        },
    });
    ...
    const handleUpdateProject = (data: { category: string; total_budget: number }) => {
        ...
        updateMutation.mutate({ name: selectedProject, data: { total_budget: data.total_budget } });
    };
    ...
    const projectTotalRule = projectDetails?.rules?.find((r: any) =>
        r.rule.tags?.includes('ALL_TAGS') || r.rule.tags === 'ALL_TAGS' || (Array.isArray(r.rule.tags) && r.rule.tags[0] === 'ALL_TAGS')
    );
    ...
    const initialModalData = isEditMode && projectTotalRule ? {
    ...
    const otherTransactions = useMemo(() => {
        ...
        const specificRules = projectDetails.rules.filter((r: any) => r !== projectTotalRule);
        ...
        return allTransactions.filter((tx: any) => !coveredIds.has(tx.unique_id || tx.id));
        ...
        <div className="space-y-8">
            ...
            <div className="flex items-center justify-between bg-[var(--surface)] p-4 rounded-2xl shadow-sm border border-[var(--surface-light)]">
                <div className="flex items-center gap-4">
                    <label className="font-semibold text-[var(--text-default)]">Select Project:</label>
            ...
            {selectedProject && projectDetails && (
                <div className="space-y-4">
                    ...
                    {projectDetails.rules.map((item: any) => {
                        const isTotalRule = item.rule.tags?.includes('ALL_TAGS') || item.rule.tags === 'ALL_TAGS' || (Array.isArray(item.rule.tags) && item.rule.tags[0] === 'ALL_TAGS');

                        return (
                            <BudgetProgressBar
                                key={item.rule.id}
                                label={isTotalRule ? 'Total Project Budget' : item.rule.name}
                                subLabel={isTotalRule ? 'Overall Allocation' : (Array.isArray(item.rule.tags) ? item.rule.tags.join(', ') : item.rule.tags)}
                    ...
                    {otherTransactions.length > 0 && (
                        <div className="pt-4 border-t border-[var(--surface-light)] mt-8">
                            <h3 className="text-sm font-bold text-[var(--text-muted)] mb-3 uppercase tracking-wider">Other Project Transactions</h3>
                            <BudgetProgressBar
                                label="Uncategorized Spending"
                                subLabel="(Transactions not covered by specific rules)"
                                current={otherTransactions.reduce((acc: number, tx: any) => acc + Math.abs(tx.amount || 0), 0)}
                    ...
                    {projectDetails.rules.length === 0 && (
                        <div className="text-center text-[var(--text-muted)] py-8">No budget rules defined for this project.</div>
                    )}
                </div>
            )}
