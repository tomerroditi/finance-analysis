...
import { ChevronLeft, ChevronRight, Plus, PenSquare, Trash2, Copy } from 'lucide-react';
...
export const MonthlyBudgetView: React.FC = () => {
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['budgetAnalysis'] }),
    });
    ...
    if (isLoading) return <div className="p-8 text-center text-[var(--text-muted)]">Loading budget data...</div>;
        ...
        <div className="space-y-8">
            ...
            <div className="flex items-center justify-between bg-[var(--surface)] p-4 rounded-2xl shadow-sm border border-[var(--surface-light)]">
                <div className="flex items-center space-x-4">
                    <button onClick={handlePreviousMonth} className="p-2 hover:bg-[var(--surface-light)] rounded-full text-[var(--text-muted)] hover:text-[var(--text-default)] transition-colors">
                        <ChevronLeft size={24} />
                    </button>
                    ...
                    <button onClick={handleNextMonth} className="p-2 hover:bg-[var(--surface-light)] rounded-full text-[var(--text-muted)] hover:text-[var(--text-default)] transition-colors">
                ...
                <div className="flex gap-3">
                    ...
                    <button
                        onClick={() => {
                            if (confirm('This will delete all current rules for this month and copy rules from the previous month. Continue?')) {
                                copyMutation.mutate();
                            }
                        }}
            ...
            <div className="space-y-4">
                {rules.map((item: any) => (
                    <BudgetProgressBar
                        ...
                        subLabel={item.rule.category !== item.rule.name ? item.rule.category : undefined}
            ...
            {
                project_spending && project_spending.projects && project_spending.projects.length > 0 && (
                    <div className="pt-8 border-t border-[var(--surface-light)]">
                        <h3 className="text-lg font-bold text-[var(--text-muted)] mb-4 uppercase tracking-wider text-xs">Project Spending</h3>
                        <div className="space-y-4">
                            {project_spending.projects.map((project: any) => (
                                <BudgetProgressBar
                                    ...
                                    total={project.spent}
                                    onToggleExpand={() => toggleExpand(`project_spending_${project.category}`)}
                                    isExpanded={expandedRuleId === `project_spending_${project.category}`}
                                >
                                    <TransactionCollapsibleList
