 ...
 * balance.
 *
 * Rows that match no bucket are **ignored**, never counted as cost — so a
 * provider adding a statement row cannot silently re-inflate the figure. Cost
 * buckets take only negative amounts: a positive-signed cost row is a
 * provider-format change, not a cost.
 */
export function classifyStatement(json: string | null): StatementBreakdown {
  const breakdown: StatementBreakdown = { riskCost: 0, managementFee: 0, actuarial: 0 };
