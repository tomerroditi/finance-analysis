...
export function Categories() {
    ...
    const createTagMutation = useMutation({
        mutationFn: ({ category, tag }: { category: string, tag: string }) => taggingApi.createTag(category, tag),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['categories'] });
    ...
    const deleteTagMutation = useMutation({
        mutationFn: ({ category, tag }: { category: string, tag: string }) => taggingApi.deleteTag(category, tag),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['categories'] })
    });

    const relocateTagMutation = useMutation({
        mutationFn: ({ tag, newCategory, oldCategory }: any) => taggingApi.relocateTag(oldCategory, newCategory, tag),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['categories'] });
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {categories && Object.entries(categories as Record<string, string[]>).map(([category, tags]) => (
                    <div
                        key={category}
                    ...
                    >
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <button
                                onClick={() => {
                                    if (window.confirm(`Delete category "${category}"? All associated tags will be nullified in existing transactions.`)) {
                        ...
                        <div className="flex-1 space-y-2">
                            <div className="flex flex-wrap gap-2">
                                {tags.map((tag) => (
                                    ...
                                    >
                                        <span className="text-sm font-medium text-[var(--text-muted)]">{tag}</span>
                                        <div className="flex items-center gap-0.5 opacity-0 group-hover/tag:opacity-100 transition-all ml-1">
                                            ...
                                            <button
                                                onClick={() => {
                                                    if (window.confirm(`Delete tag "${tag}" from "${category}"? It will be removed from all existing transactions.`)) {
                            ...
                            {tags.length === 0 && (
                                <div className="text-xs text-[var(--text-muted)] italic py-2">No tags defined</div>
                            )}
                        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-[var(--surface)] border border-[var(--surface-light)] rounded-2xl p-6 shadow-2xl w-full max-w-sm animate-in zoom-in-95 duration-200">
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setIsAddCategoryOpen(false)} className="flex-1 py-2 text-sm font-bold hover:text-white transition-colors">Cancel</button>
                            <button onClick={(e) => {
                            ...
                            }} className="flex-1 py-2 bg-[var(--primary)] rounded-xl text-white font-bold hover:bg-[var(--primary-dark)] transition-all">Create</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {isAddTagOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-[var(--surface)] border border-[var(--surface-light)] rounded-2xl p-6 shadow-2xl w-full max-w-sm animate-in zoom-in-95 duration-200">
                        <h3 className="text-lg font-bold mb-4">Add Tag to <span className="text-[var(--primary)]">{isAddTagOpen.category}</span></h3>
                        <input
                            autoFocus
                            type="text"
                            ...
                            className="w-full bg-[var(--surface-base)] border border-[var(--surface-light)] rounded-xl px-4 py-3 outline-none focus:border-[var(--primary)] mb-6 transition-all"
                            onKeyDown={(e) => {
                                if (e.key === 'Enter') createTagMutation.mutate({ category: isAddTagOpen.category, tag: (e.target as HTMLInputElement).value });
                                if (e.key === 'Escape') setIsAddTagOpen(null);
                            }}
                        />
                        <div className="flex gap-3">
                            <button onClick={() => setIsAddTagOpen(null)} className="flex-1 py-2 text-sm font-bold hover:text-white transition-colors">Cancel</button>
                            <button onClick={(e) => {
                                const val = (e.currentTarget.parentElement?.previousElementSibling as HTMLInputElement).value;
                                if (val) createTagMutation.mutate({ category: isAddTagOpen.category, tag: val });
                            }} className="flex-1 py-2 bg-[var(--primary)] rounded-xl text-white font-bold hover:bg-[var(--primary-dark)] transition-all">Add Tag</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {isRelocateOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-[var(--surface)] border border-[var(--surface-light)] rounded-2xl p-6 shadow-2xl w-full max-w-sm animate-in zoom-in-95 duration-200">
                        ...
                        <p className="text-sm text-[var(--text-muted)] mb-6">Move <span className="font-bold text-white">"{isRelocateOpen.tag}"</span> to a different category</p>

                        <div className="space-y-2 max-h-[200px] overflow-y-auto mb-6 pr-2">
                            {categories && Object.keys(categories).filter(c => c !== isRelocateOpen.category).map(cat => (
                                <button
                                    key={cat}
                                    onClick={() => relocateTagMutation.mutate({
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        <span>{cat}</span>
                                        <MoveRight size={16} className="opacity-0 group-hover:opacity-100 transition-all" />
                                    </div>
                                </button>
                            ))}
                        </div>
