...
- Scraping has a 5-minute timeout. The "one scrape per account per day" limit is **not enforced by the backend** — `start_scraping_single` will happily re-scrape today. What exists is a UI skip: "Scrape All" only launches accounts with no successful scrape today (per-source buttons still allow an explicit same-day re-run)
...
- **Vercel serverless (`index.py` → `backend/main.py` lifespan):** the `if os.environ.get("VERCEL"): yield; return` guard MUST stay at the very top of `lifespan`, before any import that transitively pulls in `keyring` (`scraping_service` → `credentials_repository` → `import keyring`). `keyring` is intentionally absent from the Vercel `requirements.txt` (no OS keyring in the sandbox; demo mode never scrapes), so any keyring-backed import placed above the guard crashes cold start with `ModuleNotFoundError: No module named 'keyring'` → the whole function 500s with `FUNCTION_INVOCATION_FAILED` on every route (it fails in lifespan, so it takes down all routes). The **routers** are a different story: `backend/main.py` mounts `credentials` and `scraping` inside `try/except ImportError`, so a keyring-backed import failure silently drops the whole route group (this is why the hosted demo's Data Sources page was empty). `keyring_store` and `crypto` therefore import their packages optionally — secret *reads* degrade to "nothing stored", *writes* raise — and scrape history lives in `routes/scraping_readonly.py`, which has no scraper import so it stays mounted without Playwright. Demo credentials ship in the frozen demo DB (the demo-mode toggle that used to seed them never runs on Vercel). Regression guards: `tests/backend/unit/test_vercel_lifespan.py`
