...
- **Recurring charges are confirmed, not assumed:** `RecurringService` only *detects* candidates. Each candidate carries a `confirmation` of `pending` / `confirmed` / `dismissed`, stored per normalized merchant key in `recurring_decisions`. Only **confirmed** items are acted on — the budget Overview's fixed/variable split and `committed_remaining`, the forecast's safe-to-spend, and the `newRecurring` / `priceIncrease` insight cards all read `get_confirmed_items()`. `get_recurring()` returns everything (dismissed only with `include_dismissed=True`) so the dashboard can ask. Verdicts are set through `POST /analytics/recurring/decisions`; `pending` undoes one. A verdict survives new charges, re-detection and amount drift because it is keyed by the same normalized label detection groups on. **Verdicts are written as a batch:** `set_decisions` detects once for the whole list and commits once, because every commit discards the `data_cache` generation — applying them one at a time re-ran the ~0.2 s detection per entry and made "confirm all" take seconds. The card patches its own cached summary from the verdict (`recurringOptimistic.ts`) so the row moves on click instead of after the dashboard-wide refetch, and **ended charges are hidden behind a "show ended" toggle** — they are already out of `total_monthly` and committed spend, so they are history rather than commitments.
