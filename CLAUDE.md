...
- **Recurring-charge classification:** a candidate must land in one of six non-overlapping cadence bands (biweekly / monthly / bimonthly / quarterly / semiannual / annual — each with its own tight tolerance, so a gap between two bands is *not* a cadence; **there is no weekly band** — nothing is genuinely billed every seven days, so it only ever matched habits), clear the interval-regularity gate (`_MAX_INTERVAL_MAD_CV = 0.15`, a robust median-absolute-deviation spread measured against the demo history: every genuine commitment scores ≤ 0.133, the tightest ordinary shopping 0.208), and qualify on one of two amount paths — `fixed` (≥75% of charges within ±15%) or `metered` (a consumption bill: ±50%, but needing a tighter schedule and ≥6 sightings, because a varying amount is no evidence). Biweekly candidates additionally must hold one weekday; month-scale ones get no day gate (real bills slip 5–6 days around weekends and month ends). Every candidate carries a `confidence` (0..1 over regularity, interval shape, day anchoring, amount stability and evidence count) that ranks the review list, and an `amount_kind`.
