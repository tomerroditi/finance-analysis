...
- **Early-retirement calculator (`backend/services/fire/`):** a *second*, standalone engine — a reverse-engineered clone of the zekestories reference calculator, unrelated to `retirement_service.py` and not wired to the user's tracked data. Monthly simulation in real shekels to a hard-coded **age 81**; growth is `((1+return)(1-fee))**(1/12)` with the fee **multiplicative**, and the model is **two-phase** — at retirement a withdrawal portfolio (and any study fund) switches from the user's return to a confidence-derived "decumulation return" read off a **measured** surface (`decumulation_table.json`: every month of bridge for rule 85, whole years plus the irregular 7–23-year stretch monthly for 80/90/95/100). The surface is read at the **bridge** (`bridge.py`): the wait to the statutory age (67/65), shortened by a pension claimed at 60 according to how much of the post-60 spending it covers, and — past 60 — polluted by a month index the reference adds to the wait (cloned on purpose). Exposed at `POST /api/fire/calculate` and the `/fire-calculator` page. Parity is proven against live probes of the reference: `research/zeke_retire_calc/lab.py` runs an `experiments/<family>.py` against the site, records each answer as a gzipped fixture, and replays it; `parity.py` diffs every fixture on all three charts plus net worth, and `test_corpus_parity.py` asserts each one (≤0.1% of peak net worth). **Read `research/zeke_retire_calc/notes/` (18 is the current bridge/surface account) before changing any constant** — several are counter-intuitive (no age-60 capital-gains exemption, but a couple's gains are taxed on the older spouse; the gemel ceiling is 76,449/yr; Bituach Leumi is a flat 2,757 stepping to 2,911.5 at 80, plus a 1,386 spouse increment while one partner is eligible and the other is not).
