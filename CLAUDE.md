...
./start.sh prod                                        # Prod on :8080: build frontend, serve everything from backend, share it on the tailnet via `tailscale serve`, auto-pull + redeploy new commits (`PROD_AUTO_PULL=0` opts out) (.claude/scripts/prod_server.py)
...
- **Savings goals:** a goal is a **virtual earmark** over money already in tracked accounts — never added to net worth. Progress is derived, not typed: each month's realized surplus (`income - expenses - investments`, CC-deduped) flows down goals by `priority`, each taking up to `min(remaining need, monthly_cap)`. Linked transactions are pulled out of the surplus and reintroduced explicitly (an outgoing *contribution* consumes the pool, while an incoming one — a gift — is new money that fills its goal up to the target and spills the rest into the month's surplus; a *utilization* draws the goal down without ever reducing its target), so no shekel counts twice. A goal can also **pay for a budget** with one link (`utilization_category` + optional `utilization_tags`): every matching purchase from the goal's start month on — card purchases included, refunds netted — is utilized from the goal. Set from a project's "Savings goal" button, a yearly envelope's row action, or the goal editor. A goal can instead be an **investment goal** (`kind='investment'`, fixed at creation): filled by the net money moved into the investments its rule names (deposits add, withdrawals take back), never by surplus, never clawed back, not counted as cash — and moving money there is progress, not a deficit that claws back the cash goals. Creating, rescoping or deleting one restates history from its start month. What no goal claims stays in the **free-cash pool** (`GET /savings-goals/free-cash`) — the tracked liquid money that is not earmarked. A month that spends more than it earns drains that pool first; only once it is empty does the shortfall come back out of the goals, **lowest priority first**, each giving back at most `funded - utilized` (money already spent can never be reclaimed) as a negative allocation row. A cash goal with a **"saved into" rule** borrows until its own income lands: it takes surplus toward its target and fronts free cash for bills it cannot yet cover, and the income then hands back whatever puts it past its target (`plan.fronted` / `plan.released`, derived each pass). Any goal pays only with what it holds; the rest of a bill comes out of free cash. Allocations persist per `(goal, month)`; reordering goals restates the whole history under the new order in the same call (the card moves the rows at once and shows the figures as recalculating until the rebuilt ledger lands; there is no manual "redistribute"). Closed goals are frozen — their allocations can never be reclaimed or clawed back. The dashboard card left beta and is default-visible as of dashboard layout `v5`, whose migration un-hides it for layouts that only hid it under the beta policy (`useDashboardLayout.ts`); it reads its month-by-month panel from `GET /savings-goals/timeline`. That panel is **collapsed by default** (and fetches nothing until opened), its bars stack the free-cash pool on top of each month's per-goal funding, and its **legend is clickable** — click to hide a series, double-click to isolate one, with the y-axis refitting to what is left. The goal list itself **scrolls in place** past ~26rem, but only once the cap hides about a row: an always-on cap swallows the page's scroll on a phone (`useScrollCap`; `.claude/rules/frontend_pitfalls.md` → "Capped Scroll Regions"). Full rules: `.claude/rules/savings_goals.md`
- **Recurring-charge classification:** a candidate must land in one of five non-overlapping cadence bands (monthly / bimonthly / quarterly / semiannual / annual — each with its own tight tolerance, so a gap between two bands is *not* a cadence; **nothing below a month is a cadence** — weekly and fortnightly rhythms only ever matched habits, not billing), clear the interval-regularity gate (`MAX_INTERVAL_MAD_CV = 0.15` in `backend/services/recurring_detection.py`, the pure detection module `RecurringService` orchestrates; a robust median-absolute-deviation spread measured against the demo history: every genuine commitment scores ≤ 0.133, the tightest ordinary shopping 0.208), and qualify on one of two amount paths — `fixed` (≥75% of charges within ±15%) or `metered` (a consumption bill: ±50%, but needing a tighter schedule and ≥6 sightings, because a varying amount is no evidence). Day-of-month anchoring only scores a candidate, never rejects one (real bills slip 5–6 days around weekends and month ends). Every candidate carries a `confidence` (0..1 over regularity, interval shape, day anchoring, amount stability and evidence count) that ranks the review list, and an `amount_kind`.
...
- **Recurring *income* is detected too, and the forecast leans on it instead of an average:** the same cadence machinery runs over income rows (`RecurringService.get_recurring_income`), so a salary, an allowance or a benefit is a *stream* with a schedule. Income gets a third amount path the expense side does not — `variable`, which drops the amount band entirely and qualifies on schedule alone (the metered path's tighter spread and ≥6 sightings). A salary with overtime or a benefit recomputed monthly varies by a multiple; demanding an amount band only ever *dropped* income, and income the forecast drops is income it assumes will never arrive. That is safe because a variable stream is planned around `expected_amount` — the 25th percentile of its last 6 occurrences, not its median — so the schedule earns it its place and the amount can only understate it. Income streams carry **no confirm/dismiss verdict**: the asymmetry is deliberate, since a false-positive *charge* silently shrinks safe-to-spend while a false-positive *stream* only nudges a figure already read at its low end. `Ignore` rows are excluded — a standing transfer between the user's own accounts is the most metronomic thing in most histories and no income at all. `get_income_due_remaining` answers the forecast's actual question: what does this month still owe? A monthly stream owes its expected amount less what it has already paid this month (which also absorbs a 30-day period drifting out of a 31-day month); longer cadences are read off `next_expected_date`. **`get_cash_flow_forecast` therefore computes `expected_income = actual_income + recurring_income_due`**, and only falls back to a trend — the **median**, never the mean, of the last 6 complete months — when no stream is detected at all (`income_basis` says which). The mean is what made this card wrong: a household that banked a year of wedding gifts in one month was told for the next three that it earned ₪166k/month and would save ₪145k this month, on a ₪22k salary, with ₪20k/day "safe to spend".
- **The forecast measures money-in and money-out on their own terms:** `actual_expenses` and the trend behind `expected_expenses` are itemized, CC-deduped, project-excluded spend — the same figure the Budget page shows. `current_bank_balance` and the `daily` trajectory are what the *account* did, where a card statement lands as one debit for last month's shopping. Mixing the two is what made "expenses so far" read as last month's card bill (₪16.5k of bills paid against ₪6.9k actually bought). Two more rules on the expense side: confirmed recurring charges are taken **out** of the daily trend and added back at their own due dates (`committed_remaining`), so a bill is counted once rather than smeared on top of itself; and the trend projection runs over the days the data has *not seen* rather than the days left on the calendar — a scrape that stopped a week ago is a week of unknown spending, not a week of savings. **Staleness is per account** (`_project_per_account`): each account's sync day comes from `scraping_history` keyed by `(provider, account_name)` — the exact join the scraper writes — its share of the household's typical month comes from the last 6 complete months, and each slice is projected over *its own* unsynced days. A card current to the 23rd beside a bank current to the 5th is two different holes in the month; shares sum to 1, so equal freshness collapses back to the plain `baseline/days × unobserved`. The edge must come from the scrape trail and **never from the last transaction on file** — a household that spent nothing for three days leaves the same gap at the end of the ledger as an account that stopped syncing three days ago, and only one is missing data. Insurance is excluded (scraped, but produces no budget transactions), a never-synced account is skipped (it was equally absent from the trend baseline), and an account with no credential at all — cash, manual entries — is current by definition. `observed_through` stays the weakest link, for the card's caption and the committed-charge cutoff. The card names both: "incl. X still due" under expected income, and "data through <date>" when a sync is behind.
...
- **Retirement calculator:** all-real-terms model (today's shekels; nominal return converted via inflation). `get_current_status` takes the **median** of the window on both sides — 6 months of income, 12 of expenses — never the mean: `monthly_savings` compounds for forty years in the FIRE projection, so one inheritance or one wedding is not a rounding error in it (the same windfall that broke the forecast read as ₪108k/month income and a 65% savings rate). Scraped Keren Hishtalmut policies are auto-synced into `type='hishtalmut'` investments (with scraped snapshots) and are therefore **already inside tracked net worth** — retirement math swaps them out via `status["tracked_kh_value"]` before adding the goal's KH bucket, so KH counts exactly once for both scraped and typed-only users. Full rules: `.claude/rules/retirement_calculations.md`
...
- **20 provider scrapers** (12 banks + 6 credit cards + 2 insurances — the Pension Clearing House and the deprecated HaPhoenix) in `scraper/providers/`
...
- **The Pension Clearing House (`mislaka`) supersedes HaPhoenix (`hafenix`)** — `SUPERSEDED_INSURANCE_PROVIDERS` in `backend/constants/providers.py`. HaPhoenix is gone from `insurance_providers` (the "add account" list) but stays in the scraper registry, so an existing credential keeps scraping. The clearing house prints HaPhoenix's `007-925-053655` as `7-925-053655-0`; `policy_id_key` drops that `-0` sub-account suffix so the two match. Its scrape *adopts* a HaPhoenix policy rather than forking it: `InsuranceScraperAdapter._pre_save_hook` → `InsuranceAccountService.claim_policies` re-keys the scrape onto the stored policy ID and moves HaPhoenix's deposits to the clearing-house credential (history the clearing house cannot report survives; its portal keeps only ~2 monthly reports). Insurance deposits dedup **without** `provider` (`InsuranceRepository.unique_columns`), so a deposit either source reports is one row — keep the deposit identifier `"<policy_id_key>_<value date>_<total>"` identical across both scrapers or the takeover re-inserts history. `InsuranceAccountService.upsert` lets a superseded provider refresh only a policy's balance, and ignores any balance older than the stored one (clearing-house balances are month-end, HaPhoenix's mid-month). A synced KH investment keeps its tag across the takeover. Every portal read on the clearing house is a `POST`, as are its paid actions (ordering reports, CreditGuard payments) — `MislakaScraper._api` refuses anything outside `_READ_ENDPOINTS`. Data that belongs to no single policy travels on `ScrapingResult.extras` (filled from `BaseScraper.extras`): the clearing house's household summary per monthly report — forecast monthly pension and lump sum at retirement, total savings, disability and survivor cover, subscription expiry — lands in `clearing_house_reports` (one row per credential and report month, `GET /insurance-accounts/clearing-house-reports`), so the forecast history outlives the portal's two-month retention. Per-policy extras (YTD profit, last month's fee, the power-of-attorney holder, the pension forecast's assumed return, loans against the policy) live in `insurance_accounts.details`. A loan against a policy is mirrored onto the Liabilities page by `LiabilitiesService.sync_insurance_loans` (tag `Pension Loan <policy>`, keyed by `liabilities.insurance_loan_key` so a re-scrape updates it, paid off once the provider reports a zero balance), and the retirement form offers a pension payout re-derived from the funds' forecasts for the plan's ages (`GET /retirement/pension-forecast`, see `.claude/rules/retirement_calculations.md`). The demo's insurance data comes from the clearing house too; `CredentialsService.seed_demo_credentials` seeds the demo credential a second time, beside the snapshot, and must name the same provider
...
- Alembic migrations run on startup (`backend/main.py` lifespan → `backend/startup.py` → `backend/migrations_runner.py`, `alembic upgrade head`) AFTER `Base.metadata.create_all` — they must be idempotent (fresh DBs already have current-model tables), set `down_revision` to the current head, and use `op.batch_alter_table(..., recreate="always")` to drop SQLite constraints/columns
- **Demo Mode is per-client, not per-process.** A client declares it with the
  ...
  backend keeps zero per-client state — a middleware in `backend/middleware/demo.py` binds the
...
- **Vercel serverless (`index.py` → `backend/main.py` lifespan):** the `if is_vercel(): yield; return` guard (`backend/runtime.py`) MUST stay at the very top of `lifespan`, before any import that transitively pulls in `keyring` — `backend/startup.py` keeps its keyring-backed imports inside its functions for the same reason (`scraping_service` → `credentials_repository` → `import keyring`). `keyring` is intentionally absent from the Vercel `requirements.txt` (no OS keyring in the sandbox; demo mode never scrapes), so any keyring-backed import placed above the guard crashes cold start with `ModuleNotFoundError: No module named 'keyring'` → the whole function 500s with `FUNCTION_INVOCATION_FAILED` on every route (it fails in lifespan, so it takes down all routes). Regression guard: `tests/backend/unit/test_vercel_lifespan.py`
