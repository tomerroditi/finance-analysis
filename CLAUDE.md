...
- **Vercel demo sandboxes are per-visitor and Blob-backed.** `index.py` sets `FAD_DEMO_SESSIONS=1`; the frontend sends a per-browser `X-FAD-Demo-Session` id on every request and the middleware routes demo requests to `demo_env/sessions/<id>/demo_data.db`, mirroring the whole file to Vercel Blob after each successful write (needs `BLOB_READ_WRITE_TOKEN`; without it sandboxes are instance-local). Never enable `FAD_DEMO_SESSIONS` locally or under Playwright — the e2e suite depends on one shared demo file. Full rules: `.claude/rules/vercel_demo.md`
