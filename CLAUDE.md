...
- **Early-retirement calculator (`backend/services/fire/`):** a *second*,
  ...
  **two-phase** — at retirement a withdrawal portfolio (and any study fund)
  ...
  evidence-backed in `research/zeke_retire_calc/notes/` (16 notes, 140 recorded
  fixtures); parity tests replay all 134 that carry charts month by month, with
  **no fitted input**. Everything measured off the corpus is regenerated by
  script, in this order: `fit_decumulation.py` (one rate per fixture) →
  `build_decumulation_table.py` (the shipped surface); the four annuity factors
  come from `measure_annuity_factors.py` → `tune_annuity_factors.py`, and
  changing those means re-running the first pair, whose rates depend on them.
  **Read the notes before changing any constant** — several are
