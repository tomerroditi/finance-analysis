...
class TestLastScrapesRoute:
    ...
    def test_returns_the_history_service_rows(self, test_client):
        ...
        Patching the service in ``routes/scraping_readonly`` also guards the
        mount: if the path ever drifts back into ``routes/scraping.py``, it
        disappears again on any deployment without Playwright, and the rows
        asserted here stop coming back.
        """
        rows = [
            {
                "service": "banks",
