...
class DataFixtures:
    ...
    def credentials(self) -> dict:
        ...
        with open(AppConfig().get_credentials_path(), "r") as file:
    ...
    def fake_credentials(self) -> dict:
        ...
        return {
            "credit_cards": {
                "isracard": {
                    "user1": {"id": "my_id", "card6Digits": "123456"},
                    "user2": {"id": "my_id", "card6Digits": "123456"},
                },
                "max": {
                    "user1": {"username": "my_username", "password": "my_password"},
                    "user2": {"username": "my_username", "password": "my_password"},
                },
            },
            "banks": {
                "onezero": {
                    "user1": {"email": "my_email", "password": "my_password"},
    ...
    def fake_transactions_data_maker(self, faker) -> Callable:
        ...
        def example_data(length: int = 10) -> pd.DataFrame:
            ...
            data = pd.DataFrame(
                {
                    "account_number": [faker.word()] * length,
